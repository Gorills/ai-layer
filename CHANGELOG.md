...
- Collapsed the installed native bootstrap to one procedure copy plus the engineering floor; `project_status` now returns a state snapshot (focus, continuation, git, dynamic policy, compact Work rows, map freshness) without re-sending `agent_contract`/guidance; idle ordinary continuation names `work_begin` as the next tool (including research/diagnose) instead of “handle natively”; MCP initialize instructions remain the fallback when native bootstrap is missing; `ai-layer-workflow` no longer presents itself as always-on for every registered-project chat.
