...
`AgentRun` records an observed root/subagent lifecycle for a WorkItem. A run has host/client/session/turn/model identity, heartbeat, terminal state, observability coverage and assurance. A non-stale AgentRun is required before Dashboard may claim ordinary work is live. MCP bridge traffic or an open managed Task alone does not prove user work is currently executing.
