...
17. End every completed-work response with both a concrete **What next** recommendation and a ready-to-copy, self-contained **Prompt for the next chat**. If nothing remains required, say so explicitly and offer an optional audit/publication/next-objective prompt. The prompt must require inspection of current source and Git state instead of treating handoff prose as code truth.
