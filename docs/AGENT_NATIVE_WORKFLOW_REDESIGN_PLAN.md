...
### Independent review corrections — 2026-08-20

A skeptical review against the current source, tests, migrations, accepted ADRs, and branch state found several assumptions that must be constrained before later phases. The following corrections are part of this plan and override any more general wording below:

- **Current Work links are associations, not invertible ownership.** `WorkItem.linked_task_id` and `linked_epic_id` are optional, mutable, and not unique. Current Work APIs allow links at begin or checkpoint time, and managed-Task compatibility code can select one of multiple linked Work rows. Phase 2 must therefore audit/classify legacy rows before creating reverse canonical ownership. It must never silently choose a winner among ambiguous Work rows, and a generic `linked_epic_id` must not be reinterpreted as proof that a Work row is the Epic root outcome.
- **The project-wide one-open managed Task invariant remains stronger than any per-Work target cardinality.** `active`/`blocked` managed Tasks are globally serialized per project today. Epic orchestration must continue to respect that Task-engine authority: an unrelated open Task can block/pause Epic scheduling, and Epic-owned planning must not bypass Task FSM ownership or the terminal read-only stage evidence used as the accepted Task boundary.
- **Native -> reviewed promotion must distinguish creation from dirty-tree adoption.** Existing adoption semantics require unmanaged repository changes. Escalation on a clean tree must use normal managed Task creation; escalation after unmanaged edits may use adoption with preserved pre-managed provenance. The façade must derive that choice from actual repository state rather than treating adoption as a generic promotion primitive.
- **Current Project Knowledge DRAFT is already a managed-review artifact, not an ordinary-Work candidate state.** Application-level DRAFT writes require an active delegated `implement`/`fix` stage of a managed Task; `micro` and `analysis_only` workflows cannot write it. A passing REVIEW that has Task DRAFTs requires recorded inspection of those DRAFTs in that review stage before publication, and current provenance is Task-based. Phase 6 must not let ordinary Work fabricate Task/worker context merely to reuse this DRAFT state. A pre-DRAFT candidate/provenance model or an explicitly revised Knowledge contract requires an ADR and compatibility treatment before ordinary Work can persist candidates.
- **Automatic semantic Project Map learning changes accepted ADR 0018.** Phase 6 cannot remove routine/final model-facing reconcile requirements or change semantic-map ownership merely by implementation. It must revise or supersede ADR 0018 first, while preserving scanner ownership of structural facts, source-hash validation, and the current Epic completion guarantees until the replacement contract is accepted.
- **Current host observability baseline is not full native host lifecycle coverage.** Supported project integrations currently install MCP bindings/native Skills and current observability can truthfully correlate control-plane/MCP activity, but transport activity alone is explicitly not evidence that an agent is working. Phase 0 must report host-observed capabilities as observed/configured/unsupported rather than assuming session/subagent/tool hooks exist; Phase 7 may add them only where official host evidence proves them.
- **Migration backfill must cover supported historical states, not only the current 0.14 shape.** The release manifest currently declares minimum source schema `0010_adaptive_task_workflow`, while the current head is `0018_command_project_scope`, and the PostgreSQL gate exercises that source upgrade. Phase 2 migration tests must include populated supported-source upgrades, pre-Work-spine Task/Epic rows with no Work identity, ambiguous/multiple Work links to one Task or Epic, partial/missing links, in-flight managed state, and ordinary multiple active Work. Additive rollback means application compatibility can continue reading old fields while new schema remains; it must not be described or tested as a guaranteed database downgrade.

These corrections deliberately do not select the eventual Phase 1/2/6 designs. They narrow what those designs are allowed to assume so later migration and façade work starts from the actual repository contract.
