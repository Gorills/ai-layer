    ...
    *,
    ...
    with session_scope() as db:
        ...
        if not stage.worker_id:
            raise RuntimeError("STAGE_NOT_DELEGATED: verification requires an explicitly delegated active stage.")
        request = VerificationRequest.from_values(
            command, cwd=cwd, timeout_seconds=timeout_seconds, environment=environment,
        )
        ...
        append_event(
            db, event_type="VerificationStarted", project=project, aggregate_type="task_stage",
            aggregate_id=str(stage.id), payload={"command": list(request.command), "cwd": request.cwd},
        )
        db.commit()
        if executor is None:
        ...
        append_event(
            db, event_type="VerificationCompleted", project=project, aggregate_type="task_stage",
            aggregate_id=str(stage.id), payload={
                "verification_id": str(row.id), "assurance": result.assurance.value,
                "exit_code": result.exit_code, "timed_out": result.timed_out, "ok": result.passed,
                "evidence_ref": result.evidence_ref,
            },
        )
        db.commit()
        return {
            "id": str(row.id), "stage_id": str(stage.id), "task_id": str(task.id),
            "assurance": result.assurance.value, "ok": result.passed, "command": list(result.command),
            "cwd": result.cwd, "started_at": result.started_at.isoformat(),
            "completed_at": result.completed_at.isoformat(), "exit_code": result.exit_code,
            "timed_out": result.timed_out, "output_summary": result.output_summary,
            "evidence_ref": result.evidence_ref,
        }
