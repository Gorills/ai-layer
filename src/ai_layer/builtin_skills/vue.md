---
slug: vue
description: Vue engineering for Composition API state, refs/reactivity, computed values, watchers, component contracts and lifecycle-safe async behavior.
kind: stack
keywords:
- vue
- composition api
...
Use when implementing, reviewing or refactoring Vue code in a project that actually uses this stack. Detect the project version, configuration and established architecture first; combine this skill with domain skills such as backend, frontend, database, security or testing rather than treating framework syntax as the whole design.

## Core contract

- Detect the actual Vue version and project configuration from manifests/lockfiles/runtime evidence before relying on version-sensitive APIs or defaults.

- Follow the repository's established Vue architecture, naming, dependency boundaries and tooling unless the task explicitly changes them.
...
- Use computed values for derived reactive state; use watchers for side effects/synchronization, not for maintaining copies of state.

- Keep component props readonly and communicate changes through emits/model contracts or shared owner state.

- Understand ref/reactive unwrapping boundaries and avoid destructuring that accidentally loses reactivity unless using supported helpers.

- Use stable keys for lists based on entity identity so DOM/component state follows the correct item.

- Keep global Pinia/store state for genuinely shared durable client state; local UI state belongs near the component.

- Cleanup watchers/listeners/timers/async effects using lifecycle/onCleanup mechanisms appropriate to the pinned Vue version.

## Evidence to inspect

- Manifest/lockfile and runtime output proving the Vue and toolchain versions.

- Vue version, SFC/compiler/build tooling and whether project uses Options or Composition API.

- Refs/reactive/computed/watch usage, Pinia/store ownership and component props/emits contracts.

- Lifecycle hooks/subscriptions and template key usage.
...
- If a watcher only computes another value, replace it with computed.

- If state exists in props plus local mirror plus store, choose one owner and use explicit editing draft only when product behavior requires it.

- If destructuring a reactive object is needed, preserve refs/reactivity intentionally based on the project's Vue version/helpers.

## Workflow

1. Detect Vue version, project layout, config, package manager and canonical commands.
...
- Use typed `defineProps`/`defineEmits` or project conventions to make component contracts explicit.

- Use composables for reusable reactive behavior with clear inputs/outputs/cleanup, not as hidden global dependency bags.

- Use `computed` setters or `v-model` contracts only when two-way semantics are intentional and named.

- For async watchers, cancel/ignore stale requests through cleanup/request identity.

- Use provide/inject for scoped dependency/context, not as an invisible application-wide service locator.
...
- Watcher chain: several watchers sync derived state and create loops. Replace with computed/one authority.

- Prop mutation: child modifies incoming object and surprises owner. Emit/update owner or clone explicit draft.

- Lost reactivity: destructured reactive property stops updating. Preserve refs appropriately.

- Index key: state/DOM reused for wrong list item. Use stable key.

- Store reflex: every component flag enters Pinia. Keep narrow ownership.
...
- Run Vue type checker/linter/build configured by project.

- Test list reordering and component local state identity.

- Exercise watcher cleanup/stale async response behavior.

- Test props/emits/v-model contract and store reset/isolation.

- Render and inspect transitions/conditional branches for stale DOM/focus state.

- Run the project's canonical Vue formatter/linter/static or type checks.
...
- The implementation is idiomatic for the project's actual Vue version and architecture.
