...
def skill_list_cli(
    project: str | None = typer.Option(
        None, "--project", help="Include project-scoped skills for this registered project."
    ),
):
    """List available skills and registry provenance."""
    root = str(normalize_root(project)) if project else None
    records = {
        (str(x.get("scope")), str(x.get("slug"))): x
        for x in skill_records(project_root=root, include_disabled=True)
    }
    items = []
...
def skill_info_cli(
    ...
    project: str | None = typer.Option(
        None, "--project", help="Resolve project-scoped skill identity."
    ),
):
...
def skill_add_cli(
    ...
    project: str | None = typer.Option(
        None, "--project", help="Registered project root for project scope."
    ),
    slug: str | None = typer.Option(
        None, "--slug", help="Override slug for a single-document import."
    ),
    description: str | None = typer.Option(None, "--description"),
    task_term: list[str] = typer.Option([], "--task-term", help="Repeatable task-routing term."),
    always: bool = typer.Option(
        False, "--always", help="Make this project/global skill always a routing candidate."
    ),
    ...
    approve: bool = typer.Option(
        False, "--approve", help="Install after validation instead of leaving quarantined."
    ),
    replace: bool = typer.Option(
        False, "--replace", help="Replace the same registry-managed slug in this scope."
    ),
    allow_high_risk: bool = typer.Option(
        False, "--allow-high-risk", help="Explicitly install a high-risk skill after review."
    ),
):
        ...
        echo(
            {
                "status": "quarantined",
                "imports": previews,
                "next": "Review risk/issues, then rerun with --approve or install via MCP skill_install.",
            }
        )
        return
    installed = [
    ...
    echo(
        {
            ...
            "install": "ai-layer skill add catalog:<slug> --approve",
        }
    )
...
def skill_update_cli(
    ...
    wanted_scope, root = _skill_scope_root(scope, project)
    echo(
        manager_update_skill(
            slug, scope=wanted_scope, project_root=root, allow_high_risk=allow_high_risk
        )
    )
...
def skill_inbox_cli(
    ...
    if not approve:
        echo(
            {
                "inbox": str(get_settings().skill_inbox_dir),
    ...
    installed = [
        install_import(
            x["import_id"], approve=True, allow_high_risk=allow_high_risk, project_root=root
        )
        for x in previews
    ]
