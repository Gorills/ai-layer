...
def _machine_state(deps: DoctorDependencies) -> dict:
    ...
    return {
        ...
        "registry": {
            "path": str(settings.projects_registry_file),
            "count": len(deps.list_registered_projects()),
        },
...
def _selected_roots(
    deps: DoctorDependencies, *, path: str | None, all_projects: bool, machine_only: bool
) -> list[Path]:
    if machine_only:
        return []
    roots: list[Path] = []
    if path:
        ...
        deps.get_registered_project(Path.cwd()) is not None
        and deps.project_config_path(Path.cwd()).exists()
    ):
    ...
    item.update(
        {
            ...
            "drift": config.get("integration_template_version")
            != deps.integration_template_version,
...
def _machine_issues(
    deps: DoctorDependencies, machine: dict, *, runtime_ready: bool, db_ready: bool
) -> list[dict]:
    issues: list[dict] = []
    if not runtime_ready:
        issues.append(
            {
                "severity": "error",
                "problem": "stable runtime launcher is missing",
                "action": "re-run ./install.sh from the release archive",
            }
        )
    if not machine["docker_compose"]["available"]:
        issues.append(
            {
                "severity": "error",
                "problem": "Docker Compose is unavailable",
                "action": "install Docker + compose plugin, then run ai-layer upgrade",
            }
        )
    if not db_ready:
        issues.append(
            {
                "severity": "error",
                "problem": "PostgreSQL/pgvector is not ready",
    ...
    for provider, state in machine["global_bootstrap"].items():
        if not state.get("ready"):
            issues.append(
                {
                    "severity": "warning",
                    "problem": f"global {provider} bootstrap instruction is missing",
                    "action": "ai-layer upgrade",
                }
            )
            continue
...
def _service_issues(service_state: dict) -> list[dict]:
    ...
    if autostart.get("supported") and not autostart.get("enabled"):
        issues.append(
            {
                "severity": "warning",
                "problem": "always-on AI Layer service autostart is not enabled",
                "action": "ai-layer service install",
            }
        )
        ...
        issues.append(
            {
                "severity": "warning",
                "problem": "always-on AI Layer service is enabled but not reachable",
                "action": "ai-layer service restart",
            }
        )
    ...
    if service_state.get("running") and core.get("status") == "degraded":
        ...
        issues.append(
            {
                "severity": "warning",
                "problem": f"persistent AI Layer core runtime is {core.get('status')}",
                "action": "ai-layer health",
            }
        )
    return issues
...
def _strict_private_issue(item: dict) -> dict:
    footprint, changed, guard = (
        item.get("footprint", {}),
        item.get("privacy_check", {}),
        item.get("git_guard", {}),
    )
    details = {
    ...
    if details["repository_ai_artifacts"]:
        manual.append(
            "run ai-layer repair --path on this root; remaining artifacts are shown above"
        )
    return {
        "severity": "error",
    ...
    for item in projects:
        if not item.get("exists"):
            issues.append(
                {
                    "severity": "warning",
                    "problem": f"registered project path no longer exists: {item['root']}",
