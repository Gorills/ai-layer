...
def skill_search(query: str, project_root: str | None = None, limit: int = 12) -> list[dict]:
    ...
    with mcp_audit(root, "skill_search", arg_keys=["query", "project_root", "limit"]) as audit:
        ...
        for skill in all_skills(root):
            ...
            score = sum(3 if term in str(skill.get("slug", "")).casefold() else 1 for term in terms if term in haystack)
            if score:
...
def skill_get(slug: str, project_root: str | None = None, section: str | None = None) -> dict:
    ...
    with mcp_audit(root, "skill_get", arg_keys=keys) as audit:
        ...
        if not skill:
            raise ValueError(f"skill_get: unknown skill `{slug}`. Use skill_list only if explicit discovery is needed.")
...
def skill_project_create(
    ...
    with mcp_audit(root, "skill_project_create", arg_keys=["slug", "content", "project_root", "description", "task_terms", "always", "replace"]) as audit:
        with session_scope() as db:
            _project(db, root)
        ...
        audit["metrics"] = {"skill_slug": result.get("slug"), "scope": "project", "risk": result.get("risk")}
...
def skill_import(
    ...
    with mcp_audit(root, "skill_import", arg_keys=["source", "content", "scope", "project_root", "slug", "description", "task_terms", "always", "source_member"]) as audit:
        with session_scope() as db:
            _project(db, root)
        ...
        audit["metrics"] = {"imports": len(previews), "scope": wanted_scope, "risks": [x.get("risk") for x in previews]}
        return {
            "scope": wanted_scope,
            "project_root": root,
            "status": "quarantined",
...
def skill_install(
    ...
    with mcp_audit(root, "skill_install", arg_keys=["import_id", "approve", "project_root", "replace", "allow_high_risk"]) as audit:
        with session_scope() as db:
            _project(db, root)
        ...
        audit["metrics"] = {"skill_slug": result.get("slug"), "scope": result.get("scope"), "risk": result.get("risk")}
...
def skill_update(
    ...
    wanted_scope = (scope or "project").strip().casefold()
    with mcp_audit(root, "skill_update", arg_keys=["slug", "scope", "project_root", "allow_high_risk"]) as audit:
        with session_scope() as db:
            _project(db, root)
...
def skill_set_enabled(
    ...
    wanted_scope = (scope or "project").strip().casefold()
    with mcp_audit(root, "skill_set_enabled", arg_keys=["slug", "enabled", "scope", "project_root"]) as audit:
        with session_scope() as db:
            _project(db, root)
        result = set_skill_enabled(slug, scope=wanted_scope, enabled=bool(enabled), project_root=root if wanted_scope == "project" else None)
...
def skill_remove(slug: str, scope: str = "project", project_root: str | None = None) -> dict:
    ...
    with mcp_audit(root, "skill_remove", arg_keys=["slug", "scope", "project_root"]) as audit:
        with session_scope() as db:
            _project(db, root)
        result = manager_remove_skill(slug, scope=wanted_scope, project_root=root if wanted_scope == "project" else None)
