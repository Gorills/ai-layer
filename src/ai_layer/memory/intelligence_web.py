...
def frontend_topology(
    root: Path, rows: Iterable[object], dependencies: dict[str, list[str]]
) -> dict:
    paths = _paths(rows)
    ...
    component_roots = sorted(
        {
            ...
            for p in paths
            if any(
                part.casefold() in {"components", "views", "pages", "routes", "screens"}
    ...
    route_files = sorted(
        p
        for p in paths
        if Path(p).name.casefold()
        in {"router.ts", "router.js", "routes.ts", "routes.js", "app-router.tsx", "app.tsx"}
        or any(part.casefold() in {"pages", "routes", "app"} for part in Path(p).parts[:-1])
        and Path(p).suffix.casefold() in {".tsx", ".jsx", ".vue", ".svelte"}
    )[:40]
    return {
        "present": bool(
            frameworks
            or any(
                Path(p).suffix.casefold()
                in {".html", ".css", ".scss", ".js", ".jsx", ".ts", ".tsx", ".vue", ".svelte"}
                for p in paths
            )
        ),
...
def design_profile(
    root: Path, rows: Iterable[object], dependencies: dict[str, list[str]], frontend: dict
) -> dict:
    paths = _paths(rows)
    preferred = [
        p
        ...
        and any(
            hint in p.casefold()
            for hint in ("theme", "token", "variable", "style", "global", "app", "base")
        )
    ]
    fallback = [
        p for p in paths if Path(p).suffix.casefold() in {".css", ".scss", ".sass", ".less"}
    ]
    ...
    token_signal = bool(custom_properties) or any(
        "tailwind" in item for item in frontend.get("styling", [])
    )
    return {
        ...
        "design_system_signal": "explicit_tokens"
        if token_signal
        else "conventions_only_or_unknown",
...
def seo_profile(
    ...
    candidates = [
        ...
        in {".html", ".htm", ".jsx", ".tsx", ".vue", ".svelte", ".js", ".ts", ".php", ".py"}
        and not any(
            part.casefold() in {"tests", "test", "spec", "specs", "__tests__"}
    ...
    public_surface = bool(
        frontend.get("present") and (robots or sitemaps or any(marker_paths.values()) or ssr)
    )
    return {
...
def documentation_profile(root: Path, rows: Iterable[object]) -> dict:
    ...
    docs = sorted(
        ...
        and (
            ...
            .name.casefold()
            .startswith(
                ("readme", "changelog", "contributing", "deploy", "runbook", "architecture")
            )
        )
    )[:120]
    env_examples = sorted(
        p
        for p in paths
        if Path(p).name.casefold() in {".env.example", ".env.sample", ".env.template"}
    )[:20]
