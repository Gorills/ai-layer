...
def _compact_freshness(freshness: dict, source_pointers: list[str]) -> dict:
    ...
    return {
        ...
        "read_contract": freshness.get("read_contract") or (
            "Current repository source is authoritative. Verified knowledge is invalidated when its supporting evidence changes."
        ),
    }


def _audit_materials(db: Session, project: Project, knowledge_state: dict, intelligence: dict) -> dict:
    verified = list_knowledge(db, project, status="VERIFIED", limit=200) if knowledge_state["verified"] else []
    stale = list_knowledge(db, project, status="STALE", limit=200) if knowledge_state["stale"] else []
    return {
        "knowledge": [],
        "stale": [],
...
def _task_materials(
    ...
    knowledge = (
        ...
        if knowledge_state["verified"] else []
    )
    stale = (
        _compact_context_hits(search_knowledge(db, project, task, status="STALE", limit=2), total_chars=1600)
        if knowledge_state["stale"] else []
    )
    return {
        "knowledge": knowledge,
        "stale": stale,
...
def memory_context(db: Session, project: Project, task: str, limit: int = 4, *, task_runtime: dict | None = None) -> dict:
    ...
    payload = {
        ...
        "skill_access": {
            "routing_owner": "host-native", "authoritative_store": "ai-layer",
            "retrieval_tool": "skill_get", "automatic_domain_skill_injection": False,
        },
        "policy": policy_text,
