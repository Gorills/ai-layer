...
ALLOWED_PACKAGE_SUFFIXES = {
    ...
    ".py", ".js", ".ts", ".css", ".html", ".svg", ".jinja", ".j2", ".tmpl",
}
ALLOWED_PACKAGE_NAMES = {"license", "license.md", "notice", "notice.md"}
SAFE_SOURCE_TYPES = {"local-file", "local-directory", "zip", "stdin", "inline", "agent-authored", "url", "catalog"}
...
HIGH_RISK_PATTERNS: tuple[tuple[str, str], ...] = (
    (r"(?i)ignore\s+(?:all\s+)?(?:previous|higher[- ]level|project|repository)\s+(?:instructions|rules|policy)", "asks worker to ignore governing instructions"),
    (r"(?i)(?:bypass|disable|circumvent)\s+(?:the\s+)?(?:ai[- ]layer|task layer|review|security|privacy)\b", "asks worker to bypass AI Layer/security controls"),
    (r"(?i)(?:print|dump|send|upload|exfiltrat\w*)\s+.*(?:environment|env vars?|secrets?|credentials?|tokens?|ssh keys?)", "asks worker to expose host secrets/credentials"),
    (r"(?i)curl\s+[^\n|]+\|\s*(?:sh|bash|zsh)\b", "contains remote shell-pipe execution"),
    (r"(?i)(?:cat|read)\s+.*(?:\.ssh|id_rsa|id_ed25519|credentials)", "asks worker to read credential files"),
)
...
PACKAGE_SCRIPT_HIGH_RISK_PATTERNS: tuple[tuple[str, str], ...] = (
    (r"(?i)\b(?:subprocess\.|os\.system\(|popen\()", "package script can spawn arbitrary host processes"),
