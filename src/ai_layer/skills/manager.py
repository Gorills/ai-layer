...
from ai_layer.skills.common import (
    _atomic_json, _atomic_write, _sha_bytes, _sha_text, _utcnow, builtin_skill_dir,
    skill_import_dir as _import_dir,
)
from ai_layer.skills.constants import (
    ALLOWED_PACKAGE_NAMES, ALLOWED_PACKAGE_SUFFIXES, DEFAULT_SKILL_CATALOG, HIGH_RISK_PATTERNS,
    MAX_ARCHIVE_BYTES, MAX_ARCHIVE_EXPANDED_BYTES, MAX_ARCHIVE_FILES, MAX_ARCHIVE_MEMBER_BYTES,
    MAX_SKILL_BYTES, MEDIUM_RISK_PATTERNS, PACKAGE_SCRIPT_HIGH_RISK_PATTERNS, SAFE_SOURCE_TYPES,
    SLUG_RE, VALID_SCOPES, VALID_STATUS,
)
from ai_layer.skills.contracts import (
    _first_heading, _frontmatter, _infer_terms, _slugify, normalize_skill_text, validate_skill_text,
)
...
from ai_layer.skills.registry import (
    _project_identity, _record_key, _registry_lock, _skill_target, _write_registry, disabled_global_skill_slugs,
    find_skill_record, load_skill_registry, project_skill_dir, project_skill_slugs, skill_records,
)
...
from ai_layer.skills.packages import (
    _decode_document, _package_files_for_doc, _package_risk_issues, _source_documents,
)
def import_skills(
    ...
    *,
    ...
    if source_member:
        ...
        if len(matches) != 1:
            ...
            raise ValueError(f"source_member must resolve to exactly one skill document; matches={len(matches)} available={available}")
        docs = matches
    ...
    for index, (name, data) in enumerate(docs):
        ...
        meta = {
            ...
            "created_at": _utcnow(),
            "compatibility_warnings": (["always=true is compatibility-only; native hosts own skill activation. Put truly always-on policy in host Rules."] if always else []),
        }
    ...
    if duplicates:
        for item in previews:
            ...
            import shutil
            shutil.rmtree((_import_dir() / item["import_id"]).with_suffix(".assets"), ignore_errors=True)
...
def _builtin_slugs() -> set[str]:
    return {item.name.removesuffix(".md") for item in builtin_skill_dir().iterdir() if item.name.endswith(".md")}
...
def _install_package_assets(import_id: str, *, scope: str, slug: str, project_id: str | None, normalized_text: str) -> tuple[Path | None, int, int]:
    import shutil
...
def install_import(
    ...
    *,
    ...
    if not approve:
        raise RuntimeError("Skill import is quarantined. Explicit approval is required before installation.")
    if validation["risk"] == "high" and not allow_high_risk:
        raise RuntimeError("High-risk skill requires explicit allow_high_risk approval after reviewing validation issues.")
    ...
    if scope == "project":
        ...
        if project_id != str(meta.get("project_id")) or root != stored_root:
            raise RuntimeError("Project-scoped skill import identity no longer matches the target project")
    else:
        project_id = None
        root = None
    ...
    with directory_lock(_registry_lock()):
        ...
        existing = next((x for x in registry["skills"] if isinstance(x, dict) and _record_key(x) == (scope, project_id, slug)), None)
        if existing is not None and not replace:
            raise RuntimeError(f"Skill `{slug}` is already installed in {scope} scope; pass replace=True to update it.")
        ...
        registry["skills"] = [x for x in registry["skills"] if not (isinstance(x, dict) and _record_key(x) == (scope, project_id, slug))]
...
def create_project_skill(
    ...
    *,
    ...
    return install_import(preview["import_id"], approve=True, replace=replace, project_root=project_root)


def set_skill_enabled(slug: str, *, scope: str, enabled: bool, project_root: str | Path | None = None) -> dict:
    ...
    with directory_lock(_registry_lock()):
        registry = load_skill_registry()
        record = next((x for x in registry["skills"] if isinstance(x, dict) and _record_key(x) == (scope, project_id, slug)), None)
        if record is None:
...
def remove_skill(slug: str, *, scope: str, project_root: str | Path | None = None) -> dict:
    ...
    with directory_lock(_registry_lock()):
        ...
        import shutil
        shutil.rmtree(_package_target(scope=scope, slug=slug, project_id=project_id), ignore_errors=True)
        registry["skills"] = [x for x in registry["skills"] if not (isinstance(x, dict) and _record_key(x) == (scope, project_id, slug))]
        _write_registry(registry)
    from ai_layer.skills.native import sync_native_after_skill_change
...
def skill_manager_info(slug: str, *, project_root: str | Path | None = None) -> dict | None:
    ...
    for record in load_skill_registry().get("skills", []):
        if isinstance(record, dict) and record.get("scope") == "global" and record.get("slug") == slug:
            return dict(record)
    return None


def update_skill(slug: str, *, scope: str, project_root: str | Path | None = None, allow_high_risk: bool = False) -> dict:
...
def inbox_sources() -> list[Path]:
    ...
    try:
        ...
        pass
    return [item for item in sorted(path.iterdir()) if not item.name.startswith(".") and (item.is_dir() or item.suffix.casefold() in {".md", ".zip"})]
