...
def test_uninstaller_cleans_owned_integrations_before_runtime_removal():
    ...
    assert text.index("uninstall-integrations") < text.index('remove_owned_runtime_pointer "$RUNTIME_HOME/current"')
...
def test_uninstaller_preserves_unowned_files_and_launchers(tmp_path: Path):
    ...
    proc = subprocess.run(["bash", str(root / "uninstall.sh")], env=env, capture_output=True, text=True, check=False)
...
def test_uninstaller_purge_requires_recognizable_state_and_preserves_unknown_content(tmp_path: Path):
    import os
    import subprocess
    ...
    refused = subprocess.run(["bash", str(root / "uninstall.sh"), "--purge"], env=env, capture_output=True, text=True, check=False)
    assert refused.returncode != 0
    assert "not recognizably AI Layer-owned" in refused.stderr
    assert unknown.exists()

    (state / "install.json").write_text('{"version":"0.8.0","runtime_home":"/tmp/example"}\n', encoding="utf-8")
    (state / "runtime").mkdir()
    (state / "runtime" / "alembic.ini").write_text("owned\n", encoding="utf-8")
    (state / "runtime" / "docker-compose.yml").write_text("owned\n", encoding="utf-8")
    purged = subprocess.run(["bash", str(root / "uninstall.sh"), "--purge"], env=env, capture_output=True, text=True, check=False)
...
def test_installer_refuses_unowned_launcher_before_runtime_changes(tmp_path: Path):
    ...
    proc = subprocess.run(["bash", str(root / "install.sh"), "--skip-db"], env=env, capture_output=True, text=True, check=False)
...
def test_installer_keep_releases_requires_value():
    ...
    root = Path(__file__).parents[1]
    proc = subprocess.run(["bash", str(root / "install.sh"), "--keep-releases"], capture_output=True, text=True, check=False)
...
def test_installer_refuses_nonempty_unrecognized_state_home(tmp_path: Path):
    ...
    sentinel.write_text("keep\n", encoding="utf-8")
    env = {**os.environ, "HOME": str(home), "AI_LAYER_RUNTIME_HOME": str(runtime), "AI_LAYER_BIN_DIR": str(bindir), "AI_LAYER_HOME": str(state)}
    proc = subprocess.run(["bash", str(root / "install.sh"), "--skip-db"], env=env, capture_output=True, text=True, check=False)
    assert proc.returncode != 0
...
def test_installer_cleans_incomplete_release_before_activation():
    ...
    assert text.index('RELEASE_ACTIVATED=1') > text.index('mv -Tf "$RUNTIME_HOME/current.next" "$CURRENT_LINK"')
