...
def test_continuation_of_existing_plan_restores_session_without_speculative_decision_search():
    guidance = build_tool_guidance(
        "Продолжи вчерашний план интеграции payment provider", "/repo", [{"score": 0.8}],
    )
...
def test_continuation_context_is_session_first_compact_and_withholds_stale_scanner(monkeypatch):
    ...
    project = SimpleNamespace(
        id="p1", name="linux-tools", root_path="/repo",
        languages={"markdown": 22}, dependencies={},
    ...
    monkeypatch.setattr(service, "_freshness_for_request", lambda *a, **k: {
        "status": "refreshing", "refreshed": False, "snapshot_available": True,
        "background_refresh": True, "refresh_job": "queued", "changed_paths": [],
        "read_contract": "last stable snapshot is stale",
    })
    monkeypatch.setattr(service, "knowledge_status", lambda *a, **k: {
        "verified": 0, "stale": 0, "draft": 0, "superseded": 0,
        "verified_categories": [], "verified_category_counts": {},
        "verified_subsystems": 0, "overview_verified": False,
        "baseline_ready": False, "onboarding_recommended": True,
    })
    monkeypatch.setattr(service, "latest_task_summary", lambda *a, **k: {
        "key": "T-0001", "goal": "Create scaffold", "status": "completed",
        "outcome": "Scaffold complete", "changed_path_count": 18,
        "changed_paths": ["pyproject.toml", "src/linux_tools/daemon.py"],
        "completed_at": "2026-08-11T03:53:19+00:00", "handoff_available": True,
        "provenance": "ai_layer_task_history_compact",
    })
    monkeypatch.setattr(
        service, "_search_memory",
        lambda *a, **k: (_ for _ in ()).throw(AssertionError("continuation must not semantic-search knowledge")),
    )
    monkeypatch.setattr(service, "dynamic_policy", lambda root, read_only=False: "policy")

    runtime = {
        "active": False, "state": "no_active_task", "project_root": "/repo",
        "latest": {
            "key": "T-0001", "goal": "Create scaffold", "status": "completed",
    ...
    assert [item["tool"] for item in payload["tool_guidance"]["recommended_calls"]] == ["session_restore"]
    assert payload["scanner_evidence"]["available"] is False
    assert payload["scanner_evidence"]["reason"] == "continuation_mode_uses_session_history_and_current_source"
    assert "wrong-old-entrypoint.md" not in repr(payload)
    assert payload["project"]["profile"] == {
        "available": False, "reason": "continuation_mode_uses_session_history_and_current_source"
    }
    assert payload["freshness"]["scanner_evidence_withheld"] is True
    assert payload["context_budget"]["mode"] == "continuation_session_first+dynamic_policy+compact_runtime"
...
def test_ordinary_task_withholds_stale_scanner_evidence_and_profile(monkeypatch):
    ...
    project = SimpleNamespace(
        id="p1", name="demo", root_path="/repo", languages={"markdown": 20}, dependencies={},
        project_intelligence={
            "stack": {"languages": ["markdown"], "frameworks": [], "manifests": []},
            "runtime": {"entrypoints": ["obsolete.md"]}, "data": {}, "testing": {}, "documentation": {},
        },
    )
    monkeypatch.setattr(service, "_freshness_for_request", lambda *a, **k: {
        "status": "refreshing", "refreshed": False, "snapshot_available": True, "changed_paths": ["pyproject.toml"],
    })
    monkeypatch.setattr(service, "knowledge_status", lambda *a, **k: {
        "verified": 0, "stale": 0, "draft": 0, "superseded": 0,
        "baseline_ready": False, "onboarding_recommended": True,
    ...
    payload = service.memory_context(
        SimpleNamespace(), project, "Добавь health endpoint",
        task_runtime={"active": False, "next_action": {"action": "create_task"}},
    )
    assert payload["scanner_evidence"] == {
        "available": False, "reason": "scanner_snapshot_not_current",
        "freshness_status": "refreshing", "snapshot_available": True,
    }
    assert payload["project"]["profile"] == {"available": False, "reason": "scanner_snapshot_not_current"}
...
def test_memory_context_payload_is_bounded_for_token_economy():
    hits = [{
        "id": "k1", "key": "food-search", "category": "subsystem", "title": "Food Search",
        "summary": "A" * 5000, "claims": ["B" * 500] * 20, "constraints": [],
        "source_pointers": ["backend/app/utils/food_search.py"], "score": 0.9,
    }]
    compact = _compact_context_hits(hits, max_chars=1800)
    assert compact == []  # oversized curated cards are skipped rather than transport-truncated into misleading prose
...
def test_memory_search_does_not_pad_results_with_near_zero_similarity():
    ...
    item = SimpleNamespace(
        id="k1", kind="project-knowledge", title="irrelevant", content="nothing related", source_path=None,
        meta={"status": "VERIFIED", "knowledge_key": "irrelevant", "category": "other", "summary": "nothing related"},
    )
    class Result:
        def all(self):
            return [(item, 0.99)]
    ...
    try:
        assert service._search_memory(DB(), project, "specific missing parser behavior", limit=8) == []
    finally:
...
def test_decision_search_does_not_refresh_repository_memory(monkeypatch):
    ...
    monkeypatch.setattr(
        service,
        "ensure_memory_fresh",
        lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("decision history must not refresh repo")),
    )
    monkeypatch.setattr(embeddings, "get_embedder", lambda: SimpleNamespace(embed=lambda texts: [[0.0] * 384]))
...
def test_memory_context_never_injects_or_plans_domain_skills(monkeypatch):
    ...
    project = SimpleNamespace(
        id="p1", name="demo", root_path="/repo", languages={"python": 10}, dependencies={},
        architecture_summary="demo architecture", project_intelligence={},
    )
    hits = [
        {
            "id": str(i), "key": f"card-{i}", "category": "subsystem", "title": f"Card {i}",
            "summary": "M" * 500, "claims": ["verified behavior"], "constraints": [],
            "source_pointers": [f"f{i}.py"], "status": "VERIFIED", "score": 0.9,
        }
        for i in range(4)
    ]
    monkeypatch.setattr(service, "_freshness_for_request", lambda db, project: {"status": "fresh", "refreshed": False})
    ...
    monkeypatch.setattr(service, "knowledge_status", lambda *args, **kwargs: {
        "verified": 4, "stale": 0, "draft": 0, "superseded": 0,
        "baseline_ready": True, "onboarding_recommended": False,
    })
    ...
    payload = service.memory_context(
        DB(), project, "fix Django parser", limit=4,
        task_runtime={"active": False, "next_action": {"action": "create_task", "tool": "task_create"}},
    )
...
def test_coverage_audit_uses_complete_inventory_without_prior_reviewer_reasoning(monkeypatch):
    ...
    project = SimpleNamespace(
        id="p1", name="trener", root_path="/repo",
        languages={"python": 10, "typescript": 20}, dependencies={},
        project_intelligence={
            "stack": {
    ...
    cards = [
        {
            "id": f"k{i}", "key": key, "category": category, "title": title,
            "summary": summary, "claims": ["claim"], "constraints": [], "unknowns": [],
            "source_pointers": [path], "status": "VERIFIED",
        }
        for i, (key, category, title, summary, path) in enumerate([
            ("project.overview", "overview", "Project Overview", "Project map", "README.md"),
            ("subsystem.backend", "subsystem", "Backend", "Backend subsystem", "backend/app/main.py"),
            ("subsystem.mobile", "subsystem", "Mobile", "Mobile subsystem", "mobile/package.json"),
        ])
    ]
    monkeypatch.setattr(service, "_freshness_for_request", lambda *a, **k: {"status": "fresh", "refreshed": False, "changed_paths": []})
    monkeypatch.setattr(service, "knowledge_status", lambda *a, **k: {
        "verified": 3, "stale": 0, "draft": 0, "superseded": 0,
        "verified_categories": ["overview", "subsystem"],
        "verified_category_counts": {"overview": 1, "subsystem": 2},
        "verified_subsystems": 2, "overview_verified": True,
        "baseline_ready": True, "onboarding_recommended": False,
    })
    monkeypatch.setattr(service, "list_knowledge", lambda *a, status="VERIFIED", **k: cards if status == "VERIFIED" else [])
    monkeypatch.setattr(service, "knowledge_audit_history", lambda *a, **k: [{
        "key": "T-0017", "goal": "Independent Project Knowledge audit",
        "completed_at": "2026-08-11T00:00:00+00:00",
        "provenance": "ai_layer_task_history_metadata_only",
    }])
    monkeypatch.setattr(service, "dynamic_policy", lambda root, read_only=False: "critical read-only policy")
    ...
    assert [item["key"] for item in brief["knowledge_inventory"]] == [
        "project.overview", "subsystem.backend", "subsystem.mobile"
    ]
    ...
    assert payload["context_budget"]["mode"] == "knowledge_audit_inventory+compact_read_only_control_plane"
...
def test_near_empty_project_context_omits_static_policy_and_workflow_dump(monkeypatch):
    ...
    project = SimpleNamespace(
        id="p-empty", name="linux-tools", root_path="/repo",
        languages={"markdown": 1}, dependencies={},
        project_intelligence={
            "stack": {"languages": ["markdown"], "frameworks": [], "manifests": []},
            "runtime": {"entrypoints": []},
            "data": {"databases": [], "caches": []},
    ...
    monkeypatch.setattr(service, "_freshness_for_request", lambda *a, **k: {
        "status": "fresh", "refreshed": True, "snapshot_available": True, "changed_paths": [],
    })
    monkeypatch.setattr(service, "knowledge_status", lambda *a, **k: {
        "verified": 0, "stale": 0, "draft": 0, "superseded": 0,
        "verified_categories": [], "verified_category_counts": {}, "verified_subsystems": 0,
        "overview_verified": False, "baseline_ready": False, "onboarding_recommended": True,
    })
    monkeypatch.setattr(service, "relevant_task_history", lambda *a, **k: [])
    monkeypatch.setattr(service, "relevant_decision_brief", lambda *a, **k: [])
    ...
    payload = service.memory_context(
        SimpleNamespace(), project, "Add first endpoint",
        task_runtime={
            "active": False, "state": "no_active_task", "project_root": "/repo",
